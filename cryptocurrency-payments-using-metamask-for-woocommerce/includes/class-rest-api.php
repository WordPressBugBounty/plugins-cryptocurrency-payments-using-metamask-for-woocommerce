...
class CpmwRestApi
{
    ...
    public function verify_transaction_handler($request)
    {
		...
		$chain_id = $this->cpmw_chain_id( esc_html( $dbnetwork ) );
		if ( $chain_id === false ) {
		...
		$tx_info = CPMW_API_DATA::verify_transaction_info( esc_html( $trasn_id ), esc_html( $dbnetwork ), $order_id ,esc_html( $total ) );
    ...
    public function save_transaction_handler($request)
    {
		...
		$db             = new CPMW_database();
		$tx_order_id = $db->cpmw_get_tx_order_id( $tx_hash );
		$order         = new WC_Order( $order_id );
	...
	public function save_transaction_data( $request ) {
       ...
       if(count($tx_order_id) > 1 || (count($tx_order_id) > 0 && (int)$tx_order_id[0] !== $order_id)){
           $order->update_status( 'wc-failed', __( 'Order has been canceled because a same transaction already exists', 'cpmw' ) );

           $error_message = __( 'A same transaction already exists in a previous order', 'cpmw' );
       ...
       if ( $get_sign !== $signature ) {
           ...
           $original_data = json_encode(
               array(
                   'order_id'         => $order_id,
                   'selected_network' => $selected_network,
                   'receiver'         => strtoupper( $order->get_meta( 'cpmw_user_wallet' ) ),
                   'amount'           => str_replace( ',', '', $saved_amount ),
                   'token_address'    => strtoupper( $order->get_meta( 'cpmw_contract_address' ) ),
               )
           );
