...
trait CPMW_HELPER {
	...
	protected function cpmwp_get_active_networks_for_currency( $currencySymbol ) {
		...
		return array(
			'contract_address' => isset( $all_tokens[ $get_network ][ $currencySymbol ] ) ? array( hexdec( $get_network ) => $all_tokens[ $get_network ][ $currencySymbol ] ) : array(),
	...
	/**
	 * Get the chain ID based on the provided hexadecimal value.
	 *
	 * @param string $id_hx_value The hexadecimal value representing the chain ID.
	 * @return int|false The corresponding chain ID or false if not found.
	 */
	public static function cpmw_chain_id( $id_hx_value ) {
		...
		if ( array_key_exists( $id_hx_value, $walletconnectNetworks ) ) {
			return $walletconnectNetworks[ $id_hx_value ];
		} else {
			return false;
		}
	}

	public static function cpmw_rpc_endpoint( $id_hx ) {
		$rpc_end_points = array(
			'0x1'      => 'https://ethereum-rpc.publicnode.com',
			'0x38'     => 'https://bsc-rpc.publicnode.com',
			'0x61'     => 'https://bsc-testnet.publicnode.com',
			'0xaa36a7' => 'https://sepolia.infura.io/v3/d675f7880e7c47cfb10fc74c5692d560',
		);

		if ( array_key_exists( $id_hx, $rpc_end_points ) ) {
			return $rpc_end_points[ $id_hx ];
		} else {
			return false;
		}
	}
