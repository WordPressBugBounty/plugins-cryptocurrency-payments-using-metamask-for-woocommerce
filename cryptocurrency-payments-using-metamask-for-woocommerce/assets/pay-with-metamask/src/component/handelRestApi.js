...
export const restUpdatePrice = async (
  total_price,
  { nonce,restUrl }
) => {
  try {
    ...
    if (total_price) {
      ...
      const apiEndpoint = `${restUrl}update-price`;
...
// Save hash response and sender id in order page
export const restApiSaveTransactionHash=async(
  { hash, from, amount, recever, token_address },
  { restUrl, nonce, id, in_crypto, signature }
)=>{
  try {
    // Define the API endpoint for saving the transaction hash
    const apiEndpoint = `${restUrl}save-transaction-data`;
...
//Send initialize payment hash response
export const restApiPrevPaymentStatus = async (
  { from, amount, recever, token_address },
  { restUrl, nonce, id, in_crypto, signature }
) => {
  try {
    // Define the API endpoint for saving the transaction
    const apiEndpoint = `${restUrl}prev-payment-status`;
